...
def main():
    ...
    # model args
    parser.add_argument("--num_factor", help="The number of latent factors to compute", type=int, default=128)
    parser.add_argument(
        "--regularization", type=float, default=0.1, help="The regularization factor to use"
    )
    parser.add_argument(
        "--alpha", type=float, default=10, help="governs the baseline confidence in preference observations"
    )
    parser.add_argument(
        "--decay_rate", type=float, default=0.1, help="decay rate for time weighting"
    )
    ...
    decay_rate = args.decay_rate  # Configurable decay rate
    ...
    model = AlternatingLeastSquares(
        ...
        alpha=alpha,
        use_gpu=False)  # CPU mode - CUDA not available in this build
