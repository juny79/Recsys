...
def main():
    ...
    parser.add_argument("--output_path", default="../output/output_ease.csv", type=str)
    ...
    # Aggregate weights (Max weight per user-item pair)
    # If user viewed (1) and purchased (5), weight should be 5.
    ...
    # For training, dense G is fine (26k x 26k is 2.7GB)
    G = X.transpose().dot(X).toarray()
    
    print("Moving G to GPU...")
    G_tensor = torch.from_numpy(G).float().to(device)
    
    # Add Regularization
    diag_indices = torch.arange(n_items).to(device)
    G_tensor[diag_indices, diag_indices] += args.l2_reg
    ...
    print("Training Complete. Generating Predictions with GPU (Sparse MM)...")
    ...
    for start_idx in tqdm(range(0, n_users, batch_size)):
        ...
        # Ensure indices make sense for batch (0..batch_size-1)
        # coo.row are 0..batch_size-1 because we sliced X
        ...
        # Set watched items to -inf
        # We can use the sparse indices to set them to -inf in the dense result
        # i[0] is rows, i[1] is cols
        # But we need to do this efficiently.
        # S_batch_tensor[i[0], i[1]] = -inf 
        # Advanced indexing works on dense tensor
        S_batch_tensor[i[0].to(device), i[1].to(device)] = -float('inf')
        
        # Top-10
        _, top_indices = torch.topk(S_batch_tensor, 10, dim=1) # (batch, 10)
        
        # Vectorized Result Construction
        top_indices_cpu = top_indices.cpu().numpy()
        
        # Map indices to item_ids
        batch_item_ids = idx2item_arr[top_indices_cpu]
        
        # Map user indices to user_ids
        batch_user_ids_col = idx2user_arr[batch_users]
        batch_user_ids = np.repeat(batch_user_ids_col, 10)
