"""
3-Way Ensemble + Phase 1+ Enhanced Boost
==========================================
3-Way에서 부스트 효과가 약간 감소한 것을 보완하기 위해
Phase 1 부스트를 살짝 강화한 버전
...
def apply_phase1_enhanced_boost(candidates_df, history_df):
    """Phase 1+ Enhanced: 3-Way에 맞게 살짝 강화된 부스트"""
    print("Applying Phase 1+ Enhanced boosts...")
...
def build_3way_ensemble(als_df, sasrec_df, xgb_df, weights, K=60):
    """3-Way 앙상블 빌드"""
    print(f"\nBuilding 3-way ensemble...")
    print(f"  Weights: ALS={weights['als']:.2f}, SASRec={weights['sasrec']:.2f}, XGB={weights['xgb']:.2f}")
    
    # Rank-based scoring
    for name, df in [('als', als_df), ('sasrec', sasrec_df), ('xgb', xgb_df)]:
...
def main():
    parser = argparse.ArgumentParser(description="3-Way + Phase 1+ Enhanced Boost")
    ...
    parser.add_argument("--output_path", default="../output/output_triplet_enhanced.csv", type=str)
    ...
    print("="*60)
    print("3-Way + Phase 1+ Enhanced Boost")
    print("Target: 0.1326 → 0.1330+")
    print("="*60)
    ...
    # 2. Ensemble
    weights = {'als': args.w_als, 'sasrec': args.w_sasrec, 'xgb': args.w_xgb}
    total_w = sum(weights.values())
    weights = {k: v/total_w for k, v in weights.items()}
    
    print(f"\n[2/4] Building ensemble...")
    candidates_df = build_3way_ensemble(als_df, sasrec_df, xgb_df, weights, args.K)
    ...
    print("\n" + "="*60)
    print(f"✅ 3-Way + Phase 1+ Enhanced completed!")
